...
% 输入参数分别为：房间尺寸x， y， z，信号源（天线）坐标（x， y， z），网格大小（输出数据的密度），单位（m）；信号的发射频率，单位（MHz）
    ...
    %% 前平面反射路径（前后左右上下的意思是：人站在这个长方体中，面朝y轴，这时的六个面分别称为前后左右上下）
    ...
    error1 = 0.05*rand(1,1);%产生每个信号的随机噪声
    error2 = 0.05*rand(1,1);
    error3 = 0.05*rand(1,1);
    error4 = 0.05*rand(1,1);
    error5 = 0.05*rand(1,1);
    error6 = 0.05*rand(1,1);
    E = E0 + E1*(1-error1) + E2*(1-error2)  + E3*(1-error3) + E4*(1-error4) + E5*(1-error5) + E6*(1-error6);%这里所有的电场强度代表的是随路径和反射的损耗倍数，再乘以衰减系数
